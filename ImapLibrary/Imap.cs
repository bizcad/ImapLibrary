...
namespace Joshi.Utils.Imap
{
    ...
    public class Imap : ImapBase
    {
        ...
        public void DeleteMessage(string sUid)
        {
        ...
        public string GetSubject(string sMessageUID)
        {
            if (!m_bIsLoggedIn)
            {
                try
            ...
            try
            {
                ...
                {
                    string contents = asMessageHeader[i].ToString();
                    if (contents.Contains("Subject"))
                    {
                        string subject = asMessageHeader[i + 1].ToString();
                        return subject.Length > 0 ? subject : "Blank Subject";
                    }
                }
            }
            catch (ImapException e)
        ...
        /// </summary>
        /// <param name="sMessageUID"> Message UID</param>
        public string GetBodyStructure(string sMessageUID, bool bFetchBody)
        {
            ImapResponseEnum eImapResponse = ImapResponseEnum.IMAP_SUCCESS_RESPONSE;
            string sCommandSuffix = sMessageUID + " " + "BODYSTRUCTURE";
            string sCommandString = IMAP_UIDFETCH_COMMAND + " " + sCommandSuffix + IMAP_COMMAND_EOL;
            ...
            try
            {
                ...
                if (eImapResponse == ImapResponseEnum.IMAP_SUCCESS_RESPONSE)
                {
                    ...
                    foreach (string sLine in asResultArray)
                    {
                        nStart = sLine.IndexOf(IMAP_BODYSTRUCTURE_COMMAND, StringComparison.Ordinal);
                        if (sLine.StartsWith(IMAP_UNTAGGED_RESPONSE_PREFIX) &&
                            (nStart != -1))
                        {
                            sBodyStruct = sLine;
                    ...
                    nStart = sBodyStruct.IndexOf(IMAP_BODYSTRUCTURE_COMMAND, StringComparison.Ordinal);
                    sBodyStruct = sBodyStruct.Substring(nStart + IMAP_BODYSTRUCTURE_COMMAND.Length);
                    int nEnd = sBodyStruct.LastIndexOf(")", StringComparison.Ordinal);
                    if (nEnd == -1)
                    {
                        throw new ImapException(ImapException.ImapErrorEnum.IMAP_ERR_FETCHBODYSTRUCT);
                    }
        ...
        public bool ParseBodyStructure(string sMessageUID, ref string sBodyStruct, XmlTextWriter oXmlBodyPart,
                            ...
                            try
                            {
                                    ...
                                    ((sData.ToLower()).IndexOf(IMAP_MESSAGE_CONTENT_TYPE.ToLower()) == -1))
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        oXmlBodyPart.WriteElementString("DATA", s);
                                    }
                                    catch (Exception ex)
                                    {
        ...
        /// <param name="sPartPrefix">Part Prefix</param>
        /// <returns>true/false</returns>
        public bool ParseBodyStructure(string sMessageUID, ref string sBodyStruct, string sPartPrefix, bool bFetchBody, out ArrayList asAttrs)
        {
            bool bMultiPart = false;
            string sTemp = "";
            asAttrs = new ArrayList();
            sBodyStruct = sBodyStruct.Trim();
            ...
            if (sBodyStruct[0] == '(')
            {
                ...
                if (ch != '(')
                {
                    ...
                    if (!GetAttrs(sMessageUID, ref sBodyStruct, sPartPrefix, bFetchBody, asAttrs, sType, sSubType, ref sTemp)) 
                        return false;
                    // MD5 CRC Error Check
                    // Don't know what to do with it
                    if (sBodyStruct[0] == ' ')
                ...
                else // MULTIPART
                {
                    ...
                    do
                    {
                        ...
                        //oXmlBodyPart.AppendChild(oXmlChildPart);
                        // parse this body part
                        if (!ParseBodyStructure(sMessageUID, ref sBodyStruct,
                            sPartNumber, bFetchBody, out asAttrs))
                        {
                            return false;
                        }
                        //oXmlBodyPart.WriteEndElement();
        ...
        private bool GetAttrs(string sMessageUID, ref string sBodyStruct, string sPartPrefix, bool bFetchBody, ArrayList asAttrs,
            string sType, string sSubType, ref string sTemp)
        {
            asAttrs.Add(IMAP_MESSAGE_CONTENT_TYPE);
            asAttrs.Add(sTemp);
            ...
            if (sTemp.ToLower() == IMAP_MESSAGE_RFC822.ToLower()) //email attachment
            {
                ...
                if (!ParseBodyStructure(sMessageUID, ref sBodyStruct, sPartPrefix, bFetchBody, out asAttrs))
                {
                    Log(LogTypeEnum.ERROR, "Failed getting Attached Message.");
                    return false;
                }
        ...
        void GetBody(string sMessageUid, string sPartNumber, ref string sData)
        {
            ...
            try
            {
                ...
                if (eImapResponse == ImapResponseEnum.IMAP_SUCCESS_RESPONSE)
                {
                    ...
                    if (lMessageSize == 0L)
                    {
                        string sLog = string.Format("InValid Message Response: Part{0}=\"{1}\"",sPartNumber, sLine + ".");
                        Log(LogTypeEnum.ERROR, sLog);
        ...
        long GetResponseSize(string sResponse)
        {
            ...
            int nEnd = sResponse.IndexOf('}',nStart + 1);
            ...
            long nSize = Convert.ToInt64(sSize.Length);
